...
class ProfileManagerFactory implements IFactory {
	...
	public function createService(IServiceManager $sm) {
		...
		// Transitioning to new base framework, which returns empty array config values as array instead of Config
		foreach($providers as $key => $value) {
			if (is_array($value) && count($value) == 0) $providers[$key] = new LegacyConfig();
		}
		...
		/** @var \WebImage\Core\LegacyDictionary $p_config */
