...
define('FPDF_FONTPATH', ConfigurationManager::get('DIR_FS_FRAMEWORK_BASE') . 'libraries/pdffont/');
...
function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $vert_align='', $fill=false, $link='')
{
...
####################################################################################################
	...
	for ($i=0; $i < count($text_lines); $i++) {
		...
		if($txt!=='')
		{
			...
			$y_center = ($this->h-($this->y+.5*$h+.3*$this->FontSize));
			$y_pos = $y_center - ($this->FontSize * $i) + (count($text_lines) * $this->FontSize / 2) - ($this->FontSize / 2);
			$y_pos = $y_pos * $k;
			
			$s.=sprintf('BT %.2f %.2f Td (%s) Tj ET',$x_pos, $y_pos, $txt2);
			if($this->underline)
