...
FrameworkManager::loadLibrary('search.idocparser');

/**
 * Implements some base functions of of IDocParser
 **/
abstract class CWI_SEARCH_AbstractDocParser implements CWI_SEARCH_IDocParser {
	
	private $rawDocument;
	private $words;
	private $title;
	
	function __construct($raw_document) {
		$this->rawDocument = $raw_document;
		$this->parse();
	}
	...
	public function getWords($remove_stop_words=true) {
		...
		if ($remove_stop_words) {
			$words = array_filter($words, array($this, 'filterWord'));
		}
		
		return $words;
	}
	
	protected function setWords(array $words) { $this->words = $words; }
	...
	public function getRawDocument() { return $this->rawDocument; }
